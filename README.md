...
In this step, we input Bitcoin ledger data in JSON format to generate the Bitcoin transaction graph consisting of two files `revmap.pkl` and `BitcoinGraph.gt`. Then, to calculate the features of labeled Bitcoin addresses, these two files are loaded in the first cell of `data_extraction.ipynb`. 
