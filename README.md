...
Next, we run the code in `data_extraction.ipynb` to extract features. First, we load the generated Bitcoin transaction graph (i.e., `revmap.pkl` and `BitcoinGraph.gt`) through the *read the graph* cell. After loading the whole graph, we input `Bitcoin_Ads_type.csv` with only Bitcoin addresses and their corresponding labels (each type of Bitcoin address is stored in different .csv files, we process each .csv file separately and generate the corresponding .csv file including all extracted features) to further compute the statistical and local structural features in different *processing indicators* cells in `data_extraction.ipynb` (we recommend using cells noted *parallel computing* to calculate features for faster processing). 
